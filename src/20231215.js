...
const DA = 1.0;
const DB = 0.5;
const feedrate0 = 0.054;
const killrate0 = 0.061;
let feedrate;
let killrate;
...
function setup() {
  ...
  for (let i = 0; i < steps; i++) {
    coltbl[i] = floor(360 / steps) * i;
  }

  for (let i = 0; i < steps; i++) {
    let i1 = int(random(steps));
    let i2 = int(random(steps));
    let w = coltbl[i1];
    coltbl[i1] = coltbl[i2];
    coltbl[i2] = w;
  }
...
function draw_image(x, y, size, cell_num) {
  ...
  if (gen == -1) {
    coloffset = dr_i * cell_num + abs(map(dr_j, 0, 4, -2, 2));
...
function drawdots(g, w) {
  ...
  for (let y = 0; y < csize; y++) {
    for (let x = 0; x < csize; x++) {      
      ...
      if (floor(v * steps) % 2 == 0) {
        hc = 0;
        sat = 0;
        br = 25;
      } else {
        hc = int(h + coltbl[(floor(v * steps) + coloffset) % steps]) % 360;
        sat = 70;
        br = 100;
      }      
      g.stroke(hc, sat, br);
