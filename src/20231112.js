...
function fillrect(g1, g2, x0, y0, w) {
  ...
  h = int(base_h + random(180)) % 360;
  g1.stroke(h, 50, 95);
  g1.strokeWeight(3);
  let freq = pow(1.2, random(0.3) + 1.5);

  if (random() > 0.5) {
    let px = 0;
    for (let theta = 360 / w; theta < 360; theta += 360 / w) {
      for (i = -1; i < 5; i++) {
        let py = w / 4 * i - sin(theta * freq) * w / (freq * 12);
        g1.point(px, py);
      }
      px++;
    }
    ...
    for (let theta = 360 / w; theta < 360; theta += 360 / w) {
      for (i = -1; i < 5; i++) {
        let px = w / 4 * i - sin(theta * freq) * w / (freq * 12);
        g1.point(px, py);
      }
      py++;
    }
  ...
  // glitch
  g1.colorMode(RGB, 255, 255, 255);
  g2.colorMode(RGB, 255, 255, 255);

  g2.image(g1, 0, 0, w, w, 0, 0, w, w);  

  n = 5 + int(random(20));
  for (let i = 0; i < n; i++) {
    let px0 = int(random(w / 2));
    let py0 = int(random(w));
    let pw = int(random(w) + w / 8);
    let ph = int(random(w / 4) + 1);
    let sx = int(random(w));
    let sy = int(random(w));

    rgbblend(g1, g2, x0, y0, px0, py0, pw, ph, (px0 + sx) % 2, (py0 + sy) % w, w);
  }

}

function rgbblend(g1, g2, x0, y0, sx, sy, w, h, dx, dy, sz) {
  let col = Array(3);
  let sfl = [0, 1, 2];
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      let sc = g1.get((sx + x) % sz, (sy + y) % sz);
      let dc = g1.get((dx + x) % sz, (dy + y) % sz);
