// 20231008
// シェーダー芸でよく見る名前のわからない流体っぽいアルゴリズム
...
function fillrect(x0, y0, s) {
  ...
  let zoom = (random(30) + 3) * 0.002;
  let imax = int(random(15) + 5);
  let col_depth = random(10) + 30;
  ...
  for (let y = 0; y < s; y++) {
    for (let x = 0; x < s; x++) {
      ...
      for (let i = 1; i < imax; i++) {
        npx += 1.9 / i * sin(i * py + cnt * 3.0 + 0.3 * i) + 4;     
        npy += 0.8 / i * cos(i * px + cnt * 2.0 + 0.3 * i) -4;
        px = npx;
        py = npy;
      }
      h = int(base_h + 20 * sin(px)) % 360;
      stroke(h,  col_depth + (col_depth * sin(py) * 0.5 + 0.5), 95);  
      strokeWeight(1);
      point(x0 + x, y0 + y);
