...
function fillrect(x0, y0, w) {
  ...
  for (let i = 0; i < n; i++) {
    let x = nrand();
    let y = nrand();
    let dst = dist(x, y, 0.5, 0.5);
    noStroke();
    let h = int(base_h + random(120)) % 360;
    let s = (1 - dst) * 40 + random(10);
    let b = 90 + dst * 5 + random(5);
    fill(h, s, b, 40);
    rect(x0 + int(x * sub) * l, y0 + int(y * sub) * l, l);
  }
