...
function cset(n, x, y, v) {
  ...
  if (v > 0) {
    cellcnt[(y + 1) * (csize + 2) + (x + 1)]++;    
    cellcnt[(y + 1 - 1) * (csize + 2) + (x + 1 - 1)]++;    
    cellcnt[(y + 1 - 1) * (csize + 2) + (x + 1 + 0)]++;    

    cellcnt[(y + 1 - 1) * (csize + 2) + (x + 1 + 1)]++;    
    cellcnt[(y + 1 + 0) * (csize + 2) + (x + 1 - 1)]++;    
    cellcnt[(y + 1 + 0) * (csize + 2) + (x + 1 + 1)]++;    
...
function cgetcnt(x, y) {
  return cellcnt[(y + 1) * (csize + 2) + (x + 1)];
}

function csetcnt(x, y, v) {
  cellcnt[(y + 1) * (csize + 2) + (x + 1)] = v;
}
...
function fillrect_init(g, x0, y0, w) {
  ...
  let h = int(base_h + random(120)) % 360;
  col0 = color(int(h + 180) % 360, 30, 80, 100);
  col1 = color(h, 3, 20, 100);
...
function drawcntdots(g, w) {
  ...
  let ps = w / (csize - 1);
  let sz = 1 + random(0.5);  
  for (let y = 0; y * sz < csize; y++) {
    for (let x = 0; x * sz < csize; x++) {
      let val = cgetcnt(x, y);
      let sval = int(val / 150) % 2;

      let hu = hue(col1);//int(360 / 7 * sval) % 360;
      let sa = 30;
      let br = constrain(20 + 80 * (val / 90), 0, 100) - 80 * sval;

      g.stroke(hu, sa, br);
      g.strokeWeight(sz * 2.5);
      g.point(x * sz * ps, y * sz * ps);
    }
  }
}
