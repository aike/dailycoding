  ...
  for (let i = 0; i < cell_num; i++) {
    for (let j = 0; j < cell_num; j++) {
      ...
      if (random() > 0.5 || depth > 1) {
        ...
        fill_or_split(x0, y0, cell_size, 3);
      }
    }
  }
  depth--;
...
function bokeh(x0, y0, size, val) {

  let gr = createGraphics(size, size);  
  gr.image(cv, 0, 0, size, size, x0, y0, size, size);
  gr.filter(BLUR, 8);

  if (random() > 0.3) {
    gr.stroke(color(0, 0, 0, 0.05));
    gr.strokeWeight(3);
    gr.noFill();
    gr.circle(size / 2, size / 2, size * 0.8 + 3);
    gr.erase();
    gr.fill(0);
    gr.circle(size / 2, size / 2, size * 0.8);    
  }
