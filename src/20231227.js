...
//const feedrate0 = 0.05;
//const killrate0 = 0.062;

//const feedrate0 = 0.048;
//const killrate0 = 0.062;

const feedrate0 = 0.052;
const killrate0 = 0.062;
  ...
  for (let y = 0; y < cell_num; y++) {
    for (let x = 0; x < ceil(cell_num / 2); x++) {
      ...
      for (let i = 0; i < imax; i++) {
        ...
        cellh[y * cell_num + x] = int(base_h + random(90)) % 360;
        cellh[y * cell_num + (cell_num - x - 1)] = cellh[y * cell_num + x];

        rx = csize / 4 + int(random(csize / 2));
        ry = csize / 4 + int(random(csize / 2));
        initdots2[y * cell_num + x].push({x:rx, y:ry});
        initdots2[y * cell_num + (cell_num - x - 1)].push({x:csize - rx, y:ry});
        
        cellh2[y * cell_num + x] = int(base_h + random(90)) % 360;
        cellh2[y * cell_num + (cell_num - x - 1)] = cellh[y * cell_num + x];      
      }       
    }
  }
...
function cget2(n, x, y, ab) {
  x = (x + csize) % csize;
  y = (y + csize) % csize;
  return cell2[n * xysize + (y + 1) * xsize + (x + 1) * 2 + ab];
}

function cset2(n, x, y, ab, v) {
  x = (x + csize) % csize;
  y = (y + csize) % csize;
  cell2[n * xysize + (y + 1) * xsize + (x + 1) * 2 + ab] = v;
}
...
function fillrect_init(g, x0, y0, w) {
  ...
  for (let y = -1; y < csize + 1; y++) {
    for (let x = -1; x < csize + 1; x++) {
      ...
      cset2((1 - xy), x, y, valA, 1);
      cset2((1 - xy), x, y, valB, 0);
    }
  }
  ...
  for (let i = 0; i < imax; i++) {
    ...
    let px2 = initdots2[current_cell][i].x;
    let py2 = initdots2[current_cell][i].y;
    for (let y = py2 - sz; y < py2 + sz; y++) {
      for (let x = px2 - sz; x < px2 + sz; x++) {
        if (dist(x, y, px2, py2) < sz) {
          cset2((1 - xy), x, y, valA, 0);
          cset2((1 - xy), x, y, valB, 0.5);          
        }      
      }
    }
  }
...
function fillrect(g, x0, y0, w) {
  ...
  for (let y = 0; y < csize; y++) {
    for (let x = 0; x < csize; x++) {      
      ...
      a = cget2(xy, x, y, valA);
      b = cget2(xy, x, y, valB);
      lap_a = laplacian2(xy, x, y, valA);
      lap_b = laplacian2(xy, x, y, valB);
      feed_a = feedrate * (1.0 - a);
      kill_b = (killrate + feedrate) * b;
      new_a = a + (DA * lap_a) - (a * b * b) + feed_a;
      new_b = b + (DB * lap_b) + (a * b * b) - kill_b;
      cset2((1 - xy), x, y, valA, new_a);
      cset2((1 - xy), x, y, valB, new_b);
    }
  }
  drawdots(g, w); 
}
...
function drawdots(g, w) {  
  ...
  let colx = int(base_h + random(60)) % 360;  
  // background
  g.fill(120, 100, 40, 100);
  g.noStroke();
  g.rect(0, 0, w, w);  
  
  g.noFill();
  g.strokeWeight(w / csize * 2);
  let c;
  let ps = w / (csize - 1);
  for (let y = 0; y < csize; y++) {
    for (let x = 0; x < csize; x++) {      
      ...
      let v1 = cget(xy, x, y, valA);
      let v2 = cget2(xy, x, y, valA);

      let v1_s = floor(v1 * 18) % 9;
      let v2_s = floor(v2 * 6) % 3;
...
function laplacian2(n, x, y, v) {
  return (
    cget2(n, x-1, y-1, v) * 0.05 + cget2(n, x, y-1, v) * 0.2 + cget2(n, x+1, y-1, v) * 0.05 + 
    cget2(n, x-1, y  , v) * 0.2  + cget2(n, x, y  , v) * -1  + cget2(n, x+1, y  , v) * 0.2  +
    cget2(n, x-1, y+1, v) * 0.05 + cget2(n, x, y+1, v) * 0.2 + cget2(n, x+1, y+1, v) * 0.05);
}
