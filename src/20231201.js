...
function fillrect_init(g, x0, y0, w) {
  ...
  col0 = color(h, 3, 100, 15);
  col1 = color(h,                  40, 90, 50);
  col2 = color(int(h + 120) % 360, 40, 90, 50);
  ...
  for (let y = 0; y < csize; y++) {
    for (let x = 0; x < csize; x++) {
      let s =
          ...
          cget(xy, x - 1, y + 1) + cget(xy, x, y + 1) + cget(xy, x + 1, y + 1)
      let num_gd1 = s % 10;
      let num_gd2 = floor(s / 10);
      let num_all = num_gd1 + num_gd2;
      let v;
      if (num_all == 2) {
        v = cget(xy, x, y);
      } else if (num_all == 3 || num_all == 4) {
        if (num_gd1 == 0 || num_gd2 == 0) {
          v = 0;       
        } else {
          v = (random() > 0.5) ? gd1 : gd2;         
        }
      } else {
        v = 0;
      }
