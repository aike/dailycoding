...
function draw_image(x, y, size, cell_num) {
  for (let i = 0; i < cell_num; i++) {
    for (let j = 0; j < cell_num; j++) {
      ...
      max_depth = int(random(6) + 4);
      theta = random(5);
      fillrect_recursive(x0, y0, cell_size);
      noFill();
      stroke('#444');
...
function fillrect_recursive(x, y, s) {
  depth++;
  stroke('#888');
  fill((base_h + int(random(150))) % 360, 20, 100);

  let hs = s / 2;
  let cx = x + hs;
  let cy = y + hs;

  let px1 = -hs; let py1 = -hs;
  let px2 =  hs; let py2 = -hs;
  let px3 =  hs; let py3 =  hs;
  let px4 = -hs; let py4 =  hs;
  
  let t = max(0, pow(depth,1.3) - 2)  * theta;
  
  beginShape();
  vertex(cx + px1 * cos(t) - py1 * sin(t),cy + px1 * sin(t) + py1 * cos(t));
  vertex(cx + px2 * cos(t) - py2 * sin(t),cy + px2 * sin(t) + py2 * cos(t));
  vertex(cx + px3 * cos(t) - py3 * sin(t),cy + px3 * sin(t) + py3 * cos(t));
  vertex(cx + px4 * cos(t) - py4 * sin(t),cy + px4 * sin(t) + py4 * cos(t));   endShape(CLOSE);
  
  if (depth < max_depth) {
    let sc = 10 / (depth + 10);
    let margin = s * (1 - sc) / 2;
    fillrect_recursive(x + margin, y + margin, s * sc);
  }
  depth--;
}
