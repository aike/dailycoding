...
class Cell {
  constructor() {}
  reset(g, black, n, x, y, w, h, bcol, wcol) {
    this.age = 0;
    this.g = g;
    this.black = black;
    this.n = n;
...
function fillrect_init(x0, y0, s) {
  ...
  for (let i = 0; i < blackcells.length; i++) {
    let x = i % 20;
    let y = floor(i / 20);
    let black = color(int(base_h + noise(r + x / 10, r + y / 10) * 120) % 360, 50, 90, 10);
    let white = color(base_h, 5, 100, 10);
    blackcells[i].reset(g, true,  i, cellsize * x + hc , cellsize * y + hc, dsize, dsize, black, white);
