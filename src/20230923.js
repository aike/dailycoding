...
function draw_image(x, y, size, cell_num) {
  ...
  let zoom = 0.12;
  cell_size = cell_size * (1 + zoom);
  let offset = cell_size * zoom * 2;
  for (let i = 1; i < cell_num - 1; i++) {
    for (let j = 1; j < cell_num - 1; j++) {
      let x0 = x + i * cell_size - offset;
      let y0 = y + j * cell_size - offset;
      base_h = int(base_h0 + random(150)) % 360;
      fillrect(x0, y0, cell_size);
      //stroke('#444');
      //noFill();
