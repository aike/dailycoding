    ...
    while (true) {
      ...
      for (let j = 0; j < nlist.length; j++) {
        ...
        if ((ny < dv) && (nx < dv) && (ny >= 0) && (nx >= 0) && (grid[ny * dv + nx] === 0)) {
          if (!acrossCheck(x0, y0, cx, cy, nx, ny, mg, ds)) {
...
function acrossCheck(x0, y0, cx, cy, nx, ny, mg, ds) {
  // 線の横断チェック
  let px0 = x0 + mg + cx * ds;
  let py0 = y0 + mg + cy * ds;
  let px1 = x0 + mg + nx * ds;
  let py1 =  y0 + mg + ny * ds;
  let checkc = get(px0 + (px1 - px0) / 2, py0 + (py1 - py0) / 2);
  if (checkc.toString() == '255,255,255,255') {
    return false;
  }
  return true;
}
