...
function fillrect(g, x0, y0, w) {
  ...
  const num_vortex = int(random(2)) + 2;
  const cx = Array(num_vortex);
  const cy = Array(num_vortex);
  const ox = Array(num_vortex);
  const oy = Array(num_vortex);
  const n = int(random(25)) + 10;
  for (let j = 0; j < num_vortex; j++) {
    cx[j] = w / 8 + random(w * 7 / 8);
    cy[j] = w / 8 + random(w * 7 / 8);
  }
  for (let i = 0; i < n; i++) {
    ...
    let th_max = 360 * (random(2) + 4);
    
    while (th < th_max) {
      for (let j = 0; j < num_vortex; j++) {
        let px = cx[j] + r * sin(th);
        let py = cy[j] - r * cos(th);
        g.line(px, py, ox[j], oy[j]);  
        ox[j] = px;
        oy[j] = py;
      }     
      r += r_delta;
