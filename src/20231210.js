...
function drawdots(g, w) {
  ...
  g.noFill();
  g.strokeWeight(w / csize * 1);
  let c;
  let ps = w / (csize - 1);
  for (let y = 0; y < csize; y++) {
    for (let x = 0; x < csize; x++) {      
      let v = cget(xy, x, y, valA);
      let vv = constrain(pow((1-v), 1.5), 0, 1); // thickness
      let n = sigmoid(vv * 1000 - 500); //
        g.stroke(h, 0, 10 + 90 * n, 100);
        g.point(x * ps, y * ps);
//      }      
    }
  }
