...
class Generator {
  constructor(w, h) {
    ...
    this.ry = this.h / 2 + random(this.h / 4);
    this.x0 = random(this.rx) + w / 2;
    this.y0 = random(this.ry) + h / 2;
...
function fillrect_init(x0, y0, s) {
  generators = [];
  bullets = [];  
  bulletsize = 0.2 + pow(2, random(2)) * 1;
...
//        for (let a = 0; a < TWO_PI; a += angleStep) {
          bullets.push(new Bullet(g, generators[i].x, generators[i].y, s, s, bulletsize, c, PI / 2, cd));
//        }
      }
    }
  }
