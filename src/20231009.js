...
function draw_image(x, y, size, cell_num) {
  ...
  base_h = int(base_h0 + random(50)) % 360;
...
function fillrect(x, y, s) {
  ...
  let f_ratio = 3;
  let ph = random(360);
  for (let th = 0; th < 360 * 8; th++) {
    let a_ratio = th / (360 * 8);
    let zoom = a_ratio * 1.0 + 0.0;
    let px0 = cx + (sin(th) * sc) * zoom;
    let py0 = cy + (sin(th * f_ratio + ph) * sc) * zoom;
    let px1 = cx + (sin(th + 180) * sc) * zoom;
    let py1 = cy + (sin((th * f_ratio + 180) + ph) * sc) * zoom;
    let a = pow(a_ratio, 2) * 90 + 10;
    let s = pow(a_ratio, 2) * 40 + 10;
    let h = int(base_h + 0 * a_ratio) % 360;
    stroke(h, s, 95, a);
    point(px0, py0);
    point(px1, py1);
  }
