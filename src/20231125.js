...
function cget(x, y) {
  return cell[(y + 1) * (csize + 2) + (x + 1)];
}

function cset(x, y, v) {
  cell[(y + 1) * (csize + 2) + (x + 1)] = v;
}
...
function fillrect_init(g, x0, y0, w) {
  ...
  col0 = color(int(h + 160) % 360, 50, 80, 20);
  col1 = color(h, 20, 100, 10);
...
function fillrect(g, x0, y0, w) {
  ...
  for (let y = 0; y < csize; y++) {
    for (let x = 0; x < csize; x++) {
      let s =
          cget(x - 1, y - 1) + cget(x, y - 1) + cget(x + 1, y - 1) +
          cget(x - 1, y    ) +                  cget(x + 1, y    ) +
          cget(x - 1, y + 1) + cget(x, y + 1) + cget(x + 1, y + 1)
      let v;
      switch (s) {
        case 2:
...
function drawdots(g, w) {
  ...
  for (let y = 0; y < csize; y++) {
    ...
    for (let x = 0; x < csize; x++) {
     ...
     g.circle((x + 1) * ps + offset, (y + 1) * ps, ps * 0.8);
    }
  }
  gen++;
}
