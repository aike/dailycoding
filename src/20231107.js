...
function fillrect(x0, y0, s) {
  ...
  for (let i = 0; i < imax; i++) {
    ...
    if (cnt % 20 == 0) {
      endShape();
      stroke(h, 40, 20 + 80 * i / imax);
      beginShape();
    }
    let r = s * 0.45 * (1 - i / imax);
    let px = x0 + s * 0.5 + r * sin(th);
    let py = y0 + s * 0.5 - r * cos(th);
    curveVertex(px, py);
    th += t + random(1);
  }
    endShape();
  console.log(log(500, 1.5));
}
