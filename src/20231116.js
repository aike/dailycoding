...
function fillrect(g, x0, y0, w) {
  ...
  for (let i = 0; i < rt; i++) {
    g.fill(h, 30 + random(20), 30 + random(30), 70);
  ...
  g.fill(h, 30 + random(20), 20 + random(20));
  g.textFont('Times New Roman', 10);
  ...
  n = 5 + int(random(10));
  for (let i = 0; i < n; i++) {
    let px0 = int(random(w) + w / 2);
    let py0 = int(random(w) + w);
    let pw = int(random(w) + w / 2);
    let ph = int(random(w / 2) + w / 32);
    let px1 = int(random(w) + w / 2);
    let py1 = int(random(w) + w);
    
    rgbblend(g, px0, py0, pw, ph, px1, py1, w);
  }

}

function rgbblend(g, sx, sy, w, h, dx, dy, sz) {
  let col = Array(3);
  let sfl = [0, 1, 2];
  let bias = [random() + 0.2, random() + 0.2, random() + 0.2];
  shuffle(sfl, true);
