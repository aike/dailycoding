...
function fillrect(g, x, y, s) {
  ...
  let h = base_h;
  let th = random(360);
  let th_delta = random(3) + 2;
  let opx = cx;
  let opy = cy;
  let mth = 0;
  let m_delta = th_delta * (random(8) + 8);
  let m_amp = 0.04;
  for (let r = 0.0; r < 2; r += 0.003) {
    let rm = r + sin(mth) * m_amp;
    let px = cx + rm * 0.8 * hs * sin(th);
    let py = cy + rm * 0.8 * hs * -cos(th);
    
    g.stroke(h, 20, 85, 100);
    let w = map(th_delta, 2, 5, 8, 2);
    g.strokeWeight(w);
    g.line(px, py, opx, opy);
    opx = px;
    opy = py;
    th += th_delta;
    mth += m_delta;
