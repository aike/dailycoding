...
class Particle {
  ...
  show(ox, oy, w, gn) {
    ...
    let ratio = w / (w + 2 * o);
    let sz = 0.7 * this.zm * (sin(gn * 0.07) + 2.5); 
    
    let hu = int(this.hu + 360 / 6 * sin(gn * 0.1)) % 360;
    let br = 100;//90 - 20 * sin(gn * 0.07);
    this.g.fill(hu, 100, br, 100);
    this.g.noStroke();
    this.g.ellipse((this.pos.x % w) * ratio + o, (this.pos.y % w) * ratio + o, sz, sz);
  }
