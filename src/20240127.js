...
class Generator {
  constructor(w, h) {
    ...
    this.w = w;
    this.h = h;
    this.rx = random(this.w / 2);
    this.ry = random(this.h / 2);
    this.x0 = random(this.w - 2 * this.rx) + this.rx;
    this.y0 = random(this.h - 2 * this.ry) + this.ry;
    this.x_theta = random(TWO_PI);
    this.x_delta = random(TWO_PI / 400);
    this.y_theta = random(TWO_PI);
    this.y_delta = random(TWO_PI / 400);
...
class Bullet {
  ...
  show() {
    ...
    let r100 = 100 - (r / this.w) * 300;
      g.fill(this.hu, this.st, this.br, r100);
      g.noStroke();
      g.circle(this.x, this.y, r * 0.1 * this.sz);
  }
...
function fillrect(x0, y0, s) {
  ...
  for (let i = generators.length - 1; i >= 0; i--) {
    generators[i].show();
    generators[i].move();
    if (generators[i].age % 150 === 1) {
      let angleStep = TWO_PI / num_shoot;
      if (bullets.length < num_shoot * 30) {
        let cr = random(360);
        for (let a = 0; a < TWO_PI; a += angleStep) {
          let c = g.color(int(cr + a / TWO_PI * 360) % 360, 60, 100);
          let cd = random(0.1);
          let blt = new Bullet(g, generators[i].x, generators[i].y, s, s, bulletsize, c, a, cd);
          blt.ox = generators[i].x;
          blt.oy = generators[i].y;
          bullets.push(blt);
        }
      }
    }
