...
function draw_image(x, y, size, cell_num) {
  ...
  let g = createGraphics(cell_size, cell_size);
...
function fillrect(g, x0, y0, w) {
  ...
  let n = int(random(50)) + 30;
  for (let i = 0; i < n; i++) {
    let h = int(base_h + random(30)) % 360;
    let s = random(15) + 30;
    let b = 95;
    let a = random(50) + 50;
    g.stroke(h, s, b, a);
    g.strokeWeight(prand(100) * 20 + 6);
    g.point(nrand() * w, nrand() * w);
  }

  // glitch
  const bmode = [BLEND,MULTIPLY, BURN, SCREEN];
  
  n = 5 + int(random(25));
  for (let i = 0; i < n; i++) {
    let px0 = int(random(w / 2));
    let py0 = int(random(w));
    let pw = int(random(w / 2) + w / 2);
    let ph = int(random(w / 4) + w / 20);
    let sx = int(random(w / 2) - w / 4);
    let sy = int(random(w / 8) - w / 16);

    g.blendMode(bmode[int(random(bmode.length))]);
    g.blend(px0, py0, pw, ph, px0 + sx, py0 + sy, pw, ph, bmode[int(random(bmode.length))]);
  }
...
function prand(crv)
{
  ...
  // 0～1の乱数を返す
  // crvはカーブの指定
  // 0.01など小さな値にすると1に近い値が返りやすい
  // 100など大きな値にすると0に近い値が返りやすい
  if (crv == 1) {
    crv = 1.0001;
  }
  const r = pow(crv, random());
  return map(r, 1, crv, 0, 1);
}
...
function nrand() 
{
  const variance = 0.17;
  const average = 0.5;
  return min(1, max(0, BoxMuller() * variance + average));  
}
