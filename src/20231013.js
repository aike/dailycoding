...
function fillrect(g, x0, y0, s) {
  ...
  const orgstyle = g.drawingContext.strokeStyle;

  const th = random(360);

  const n = int(random(120)) + 30;
  for (let i = 0; i < n; i++) {
    const px0 = random() * s * 2;
    const py0 = random() * s * 2;
    const r = random(s) + s;
    //const r = 50;
    const rth = random(20) - 10;
    //const px1 = min(s * 2, max(0, px0 + r * cos(th + rth)));
    //const py1 = min(s * 2, max(0, py0 + r * sin(th + rth)));
    const px1 = random() * s * 2;
    const py1 = random() * s * 2;
    
    const gradientStroke = 
      g.drawingContext.createLinearGradient(px0, py0, px1, py1);
    gradientStroke.addColorStop(0,   color(base_h, 40, 90, 0));
    gradientStroke.addColorStop(0.2, color(base_h, 40, 90, 100));
    gradientStroke.addColorStop(0.8, color(base_h, 40, 90, 100));
    gradientStroke.addColorStop(1,   color(base_h, 40, 90, 0));
    g.drawingContext.strokeStyle = gradientStroke;
    
    g.strokeWeight(1);
    g.line(px0, py0, px1, py1);
  }
  drawingContext.strokeStyle = orgstyle;
}

function nrand() 
{
  return min(1, max(0, (BoxMuller() + 2.5) * 0.2));  
}
