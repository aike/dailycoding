...
function fillrect(x, y, s) {
  ...
  let py_o = 0;
  let thres = s * 0.003;
  for (let th = 0; th < 360 * 40; th += 0.5) {
    let px0 = cx + (rc - rm) * cos(th) + rd * cos((rc-rm) / rm * th);
    let py0 = cy + (rc - rm) * sin(th) - rd * sin((rc-rm) / rm * th);
    ...
    th1 = th + 180;
    let px1 = cx + (rc - rm) * cos(th1) + rd * cos((rc-rm) / rm * th1);
    let py1 = cy + (rc - rm) * sin(th1) - rd * sin((rc-rm) / rm * th1);
