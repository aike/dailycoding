...
function draw_image(x, y, size, cell_num) {
  ...
  let rn = 15;
  base_h = int(360 - rn + random(rn * 2)) % 360;
...
function fillrect(g, x, y, s) {
  ...
  for (let i = 0; i < 70; i++) {
    ...
    let py = random(s);
    for (let j = 0; j < 10; j++) {
      g.fill(base_h, sat - j * delta_sat, min(100, b + j * delta_b), 30 + i / 30);
      g.circle(px - j * delta_xy, py - j * delta_xy, r - j * delta_r);
    }
    g.fill(min(30, base_h + 30), 100, 100, 30 + i / 30);
    g.circle(px - r * random(0.4) + r * 0.2, py - r * random(0.4) + r * 0.2, r * 0.45);
  }
