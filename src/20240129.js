...
class Particle {
  ...
  show(ox, oy, w) {
    ...
    let d2 = dist(this.pos.x, this.pos.y, particles[2].pos.x,  particles[1].pos.y);

    let v = min([d0, d1, d2]) / w * 20;
    
    let alp = 10 - v;
    let sat = 50;
    if (d0 < d1 && d0 < d2) {
      this.g.fill(this.hu, sat, 90, alp);    
    } else if (d1 < d0 && d1 < d2) {
      this.g.fill(int(this.hu + 60) % 360, sat, 90, alp);    
    } else {
      this.g.fill(int(this.hu + 120) % 360, sat, 90, alp);          
    }


    this.g.noStroke();
    this.g.ellipse(this.pos.x % w + ox, this.pos.y % w + oy, this.sz, this.sz);
  }
