...
function iget(n, x, y) {
  return infected[n * xysize + (y + 1) * (csize + 2) + (x + 1)];
}

function iset(n, x, y, v) {
  infected[n * xysize + (y + 1) * (csize + 2) + (x + 1)] = v;
}
...
function fillrect_init(g, x0, y0, w) {
  ...
  col0 = color(h, 100, 100, 10);
  col1 = color(h, 20, 100, 5);
  ...
  for (let y = 0; y < csize; y++) {
    for (let x = 0; x < csize; x++) {
      let s =
          ...
          cget(xy, x - 1, y + 1) + cget(xy, x, y + 1) + cget(xy, x + 1, y + 1)
      let i =
          iget(xy, x, y - 1) || iget(xy, x - 1, y    ) ||
          iget(xy, x + 1, y    ) || iget(xy, x, y + 1);
      let v;
      switch (s) {
        case 2:
          v = cget(xy, x, y);
