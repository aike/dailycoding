...
function fillrect(x, y, s) {
  ...
  let h = base_h;
  strokeWeight(1);
  let xy_ratio = random(1) + 1;
  let ph = random(360);
  let mod_ratio = random(2) + 1;
  let mod_amp = random(30) + 30;
  for (let th = 0; th < 360 * 0.7; th++) {
    let px0 = cx + sin(th + sin(th * mod_ratio) * mod_amp) * sc;
    let py0 = cy + sin(th * xy_ratio + ph) * sc;
    let px1 = cx + sin(th + 180 + sin(th * mod_ratio + 180) * mod_amp) * sc;
    let py1 = cy + sin(th * xy_ratio + 180 + ph) * sc;
    stroke(h, 40, 70, 100);
    point(px0, py0);
    point(px1, py1);
    stroke(h, 40, 100, 8);
