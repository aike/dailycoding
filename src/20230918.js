...
function fillrect_recursive(x, y, s) {
  ...
  fill(base_h, 40 - 2 * depth, min(100, 60 + 3 * depth));

  let hs = s / 2;
  let cx = x + hs + hs * 0.05 * pow(depth, 0.5) * sin(-theta * depth);
  let cy = y + hs + hs * 0.05 * pow(depth, 0.5) * cos(-theta * depth);
  
  let sc = 1 - (depth * 0.2/ max_depth);
  circle(cx, cy, s * 0.8);
