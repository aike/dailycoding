...
function fillrect(x, y, s) {
  ...
  let rc = hs * 0.7;
  let rm = rc * 0.0625 * (int(random(4)) + 4);
  let rd = rc * 0.0625 * (int(random(12)) + 5);
  
  let zoom = (rd > rm) ? pow(rm / rd, 0.5): 1;
  rc *= zoom;
  rm *= zoom;
  rd *= zoom;
  ...
  for (let th = 0; th < 360 * 2; th += 0.5) {
    let th2 = (rc-rm) / rm * th;
    let mod1_freq = th * mod_ratio;
    let mod2_freq = th2 * mod_ratio;

    let px0 = cx + (rc - rm) * cos(th + cos(mod1_freq) * mod_amp) + rd * cos(th2 + cos(mod2_freq) * mod_amp);
    let py0 = cy + (rc - rm) * sin(th + sin(mod1_freq) * mod_amp) - rd * sin(th2 + sin(mod2_freq) * mod_amp);
    
    if (th == 0) {
      px_o = px0;
      py_o = py0;
    } else {
      if (th > 360 && abs(px_o - px0) < thres && abs(py_o - py0) < thres) {
        break;
      }
    }
    ...
    th1 = th + 180;
    th2 = th2 + 180;
    mod1_freq = th * mod_ratio;
    mod2_freq = th2 * mod_ratio;
    
    let px1 = cx + (rc - rm) * cos(th + cos(mod1_freq) * mod_amp) + rd * cos(th2 + cos(mod2_freq) * mod_amp);
    let py1 = cy + (rc - rm) * sin(th + sin(mod1_freq) * mod_amp) - rd * sin(th2 + sin(mod2_freq) * mod_amp);
    /*
    stroke(h, 20, 0, 5);
    point(px0, py0);
    point(px1, py1);
