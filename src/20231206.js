...
const DA = 1.0;
const DB = 0.5;
const feedrate0 = 0.0545;//0.055;
const killrate0 = 0.0615;//0.062;
let feedrate;
let killrate;
...
function draw_image(x, y, size, cell_num) {
  ...
  if (gen == -1) {
    ...
    let rr = random(0.001);
    feedrate = feedrate0 + random(0.001);
    killrate = killrate0 + random(0.001);
...
function fillrect(g, x0, y0, w) {
  ...
  for (let y = 0; y < csize; y++) {
    for (let x = 0; x < csize; x++) {
      ...
      cset((1 - xy), x, y, valA, constrain(new_a, 0, 1));
      cset((1 - xy), x, y, valB, constrain(new_b, 0, 1));
    }
  }
  drawdots(g, w); 
}
...
function drawdots(g, w) {
  ...
  for (let y = 0; y < csize; y++) {
    for (let x = 0; x < csize; x++) {      
      ...
      g.stroke(h, 20 + 60 * v, 80 + 20 * (1 - v), 100);
      g.point(x * ps, y * ps);
    }
  }
  gen++;
