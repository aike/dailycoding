...
//
// require p5.voronoi
// https://github.com/Dozed12/p5.voronoi/tree/master
...
function fillrect(x, y, size, cell_num) {
  fill('white');
  rect(x, y, size);
  ...
  for (let i = 0; i < cell_num; i++) {
    for (let j = 0; j < cell_num; j++) {
      let c = color(0, 0, 0, 0);
      let rx = (random(cell_size) - cell_size / 2) * r;
      let ry = (random(cell_size) - cell_size / 2) * r;
      voronoiSite(j * cell_size + rx + hc, i * cell_size + ry + hc, c);
    }
  }

  voronoi(size, size, false);

  voronoiCellStrokeWeight(12);
  voronoiCellStroke(color(base_h, 40, 100, 5));
  voronoiDraw(x, y, true, false); 

  voronoiCellStrokeWeight(4);
  voronoiCellStroke(color(base_h, 40, 100, 20));
  voronoiDraw(x, y, true, false); 

  voronoiCellStrokeWeight(0.5);
  voronoiCellStroke(color(base_h, 40, 95, 90));
  voronoiDraw(x, y, true, false); 
