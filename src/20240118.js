...
class Bullet {
  ...
  show() {
    ...
    g.fill(this.hu, this.st, this.br, 30 + 30 * cos(this.age / 200));
    noStroke();
  ...
  for (let i = generators.length - 1; i >= 0; i--) {
    ...
    if (generators[i].age % 150 === 1) {
      ...
      if (bullets.length < num_shoot * 30) {
        let c = g.color(int(base_h + random(30)) % 360, 60, 100);
        let cd = 0.2 + random(0.5);
        for (let a = 0; a < TWO_PI; a += angleStep) {
          bullets.push(new Bullet(g, generators[i].x, generators[i].y, s, s, bulletsize, c, a, cd));
        }
      }
    }
  }
