...
function setup() {
  ...
  arr = Array(cell_size * 2);
  for (let i = 0; i < cell_size * 2; i++) {
    arr[i] = Array(cell_size * 2);
  }
...
function draw_image(x, y, size, cell_num) {
  ...
  let hs = cell_size / 2;
  image(g, x0, y0, cell_size, cell_size, 1, 1, cell_size, cell_size);
...
function fillrect(g, x0, y0, s) {
  ...
  const cx0 = random(s * 1.2) - s * 0.4;
  const cy0 = random(s * 1.2) - s * 0.4;
  const cx1 = random(s * 1.2) - s * 0.4;
  const cy1 = random(s * 1.2) - s * 0.4;
  const freq = random(3) + 4;
  
  for (let y = 0; y < s + 1; y++) {
    for (let x = 0; x < s + 1; x++) {
      let d0 = dist(x, y, cx0, cy0) / s * 360;
      let d1 = dist(x, y, cx1, cy1) / s * 360;
      arr[y][x] = sin(d0 * freq) + sin(d1 * freq);
      if (x > 0 && y > 0) {
        // 左上の値との差分をとり立体感を出す
        let df = arr[y][x] - arr[y - 1][x - 1];
        let h = base_h;
        let sat = 25 - 10 * df;
        let b = 85 + 15 * df;
        g.stroke(h, sat, b);
