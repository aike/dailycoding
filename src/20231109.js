...
function fillrect(g, x0, y0, s) {
  ...
  for (let i = 0; i < dv * dv; i++) {
    ...
    while (true) {
      ...
      grid[ny * dv + nx] = 1;
      g.strokeWeight(6);
      g.line(2 + mg + cx * ds, 2 + mg + cy * ds, 2 + mg + nx * ds, 2 + mg + ny * ds);
  ...
  for (let y = 1; y < s + 1; y++) {
    for (let x = 1; x < s + 1; x++) {
      let p0 = g.get(x - 1, y)[2] + g.get(x, y - 1)[2] + g.get(x - 1, y - 1)[2];
      let p1 = g.get(x + 1, y)[2] + g.get(x, y + 1)[2] + g.get(x + 1, y + 1)[2];
      let dif = (p1 - p0) / 255;
      sat = 15 - 5 * abs(dif);
      br = 75 + 15 * dif;
      strokeWeight(2);
