...
function fillrect_init(g, x0, y0, w) {
  ...
  let bg_h = int(base_h + random(120)) % 360;
  dot_h = int(bg_h + 180) % 360;
 
  // background
  g.fill(bg_h, 3, 100, 100);
  g.noStroke();
  g.rect(0, 0, w, w);
    ...
    for (let x = 0; x < csize; x++) {
      let otherval =
         (cget(xy, x - 1, y - 1) +
          ...
          cget(xy, x,     y + 1) +
          cget(xy, x + 1, y + 1)) / 8;
      let myval = cget(xy, x, y);
      let nextval
      if (otherval > 60) {
        nextval = myval + 90 - otherval;
      } else {
        nextval = myval * 0.9 + otherval * 0.1;
      }
      cset((1 - xy), x, y, nextval);
    }
  }
  drawdots(g, w); 
}
...
function drawdots(g, w) {
  ...
  for (let y = 0; y < csize; y++) {
    for (let x = 0; x < csize; x++) {
      let sat = 10 + constrain(cget(xy, x, y), 0, 100) * 0.5;
      let bri = 100 - constrain(cget(xy, x, y), 0, 100) * 0.5;
      g.stroke(dot_h, sat, bri, 100);
      g.point(x * ps, y * ps);
    }
  }
  gen++;
