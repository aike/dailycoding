...
function fillrect(x, y, s) {
  ...
  let f1_sc = s * 0.025 * (random(3) + 2);
  let f0_sc = s * 0.3 - f1_sc;
  let f1_ratio = 0.25 * int(random(5)) + 6;
  ...
  let py_o = 0;
  let cnt = 0;
  for (let f0_th = 0; f0_th < 360 * 40; f0_th += 0.5) {
    cnt++;
    let f1_th = f0_th * f1_ratio;
    let px0 = cx + sin(f0_th) * f0_sc + sin(f1_th) * f1_sc;
    let py0 = cy + cos(f0_th) * f0_sc + cos(f1_th) * f1_sc;

    if (f0_th == 0) {
      px_o = px0;
      py_o = py0;
    } else {
      if (f0_th > 360 && abs(px_o - px0) < thres && abs(py_o - py0) < thres) {
        break;
      }
    }
    
    
    let f0_th2 = f0_th + 180;
    let f1_th2 = f0_th2 * f1_ratio;
    let px1 = cx + sin(f0_th2) * f0_sc + sin(f1_th2) * f1_sc;
    let py1 = cy + cos(f0_th2) * f0_sc + cos(f1_th2) * f1_sc;
