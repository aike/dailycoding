...
function fillrect_init(g, x0, y0, w) {
  ...
  let h = int(base_h + random(90)) % 360;
  col0 = color(h, 3, 100, 1);
  col1 = color(h,                 15, 100, 20);
  col2 = color(int(h + 20) % 360, 15,  70, 20);
  col3 = color(int(h + 40) % 360, 15, 100, 20);
  //col0 = color(0, 0, 100, 1);
  //col1 = color(0, 60, 90, 20);
  //col2 = color(240, 50, 90, 20);
  //col3 = color(0, 50, 90, 20);  
...
function fillrect(g, x0, y0, w) {
  ...
  for (let y = 0; y < csize; y++) {
    for (let x = 0; x < csize; x++) {
      ...
      let v;
      if ((num_breed3 >=2 && num_breed3 <= 5) && (num_breed3 > num_breed1 && num_breed3 > num_breed2)) {
        v = breed3;
      } else if (num_breed1 >= 1 && num_breed2 >= 1) {
        v = (random() > 0.5) ? breed1 : breed2;
      } else {
        v = 0;
      }
      cset((1 - xy), x, y, v);
