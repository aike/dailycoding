...
function fillrect(x0, y0, s) {
  let sub = int(random(4, 8));
  let l = s / sub;

  let h0 = int(base_h + random(120)) % 360;
  let h1 = h0 + 120;

  let s0 = random(40) + 10;
  let s1 = max(0, s0 + (random(40) - 20));

  let b0 = random(20) + 80;
  let b1 = min(100, b0 + (random(50) - 25));
  
  for (let i = 0; i < sub; i++) {
    for (let j = 0; j < sub; j++) {
      let dst = (i + j + random(4) - 2) / (sub * 2);
      dst = max(0, min(1, dst));
      let h = map(dst, 0, 1, h0, h1);
      let s = map(dst, 0, 1, s0, s1);
      let b = map(dst, 0, 1, b0, b1);
      stroke(0, 0, 0, 10);
      fill(h, s, b);
      rect(x0 + j * l, y0 + i * l, l);
    }
  }
}
