...
function fillrect(g, g2, x0, y0, w) {
  ...
  for (let i = 0; i < rr; i++) {
    g.fill(int(h + random(100)) % 360, 20, 100);
    g.noStroke();
  ...
  for (let i = 0; i < rt; i++) {
    g.fill(int(h + 180) % 360, 40 + random(20), 60 + random(30), 50);
  ...
  g.fill(int(h + 180) % 360, 40 + random(20), 60 + random(20));
  g.textFont('Times New Roman', 10);
...
function dither(g, g2, x0, y0, w, h, step, fluc) {
  ...
  for (let y = y0; y < y0 + h + 1; y += step) {
    for (let x = x0; x < x0 + w + 1; x += step) {
      ...
      g.noFill();
      g.strokeWeight(step * 1.1);
      g.stroke(newR, newG, newB, 100);
...
function addError(g, g2, x, y, errR, errG, errB, ratio, fluc) {
  ...
  let rd = g.red(targetRGB);
  let gr = g.green(targetRGB);
  let bl = g.blue(targetRGB);
  g.stroke(rd + errR * ratio, gr + errG * ratio, bl + errB * ratio, 100);
  g.point(x + random(fluc * 2) - fluc, y + random(fluc * 2) - fluc);
}  
