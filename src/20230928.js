...
function draw_image(x, y, size, cell_num) {
  ...
  for (let i = 0; i < cell_num; i++) {
    for (let j = 0; j < cell_num; j++) {
      ...
      fillrect(x0, y0, cell_size, 100);
      // ノイズ　レイヤー２
      base_h = int(base_h + 45) % 360;
      fillrect(x0, y0, cell_size, 20);
      // 枠線
      stroke('#444');
      strokeWeight(1);
      noFill();
...
function fillrect(x, y, s, alpha_max) {
  ...
  do {
    ...
    for (let sy = 0; sy < s; sy++) {
      for (let sx = 0; sx < s; sx++) {
        ...
        let a = constrain(map(n, gmin, gmax, 0, alpha_max), 0, alpha_max);
        if (n > 0.5) {
          cnt++;
        }
        stroke(color(h, 30, 95, a));
        strokeWeight(2);
        point(x + sx, y + sy);
      }
    }
    ...
    balance = cnt / (s * s);
  } while (balance < 0.05 || balance > 0.95)
