...
function draw_image(x, y, size, cell_num) {
  ...
  for (let i = 0; i < cell_num; i++) {
    for (let j = 0; j < cell_num; j++) {
      ...
      max_depth = int(random(3) + 3);
      scale_factor = max_depth;
      ...
      fillrect_recursive(g, cell_size, cell_size, cell_size * sqrt(2));
      let hs = cell_size / 2;
...
function fillrect_recursive(g, x, y, s) {
  let h = int(base_h + random(40)) % 360;
  g.fill(h, 30, 100);
  g.circle(x, y, s - depth * cell_size / scale_factor);
  if (depth < max_depth) {
    depth++;
    fillrect_recursive(g, x, y, s);
    depth--;
  }
}
